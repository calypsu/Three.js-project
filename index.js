...
window.addEventListener("keydown", (key) => {
  ...
  if (key.keyCode === 38) H = stdForward;
    ...
    (angleDiff = -1 * stdAngleDiff), (H = stdForward);
  else if (key.keyCode === 37) (angleDiff = stdAngleDiff), (H = stdForward);

  const initialPosition = [car.position.x, car.position.y];
  const angle = car.rotation.z + angleDiff;
  const finalPosition = {};

  finalPosition.x = initialPosition[0] + -1 * H * Math.cos(angle);
  finalPosition.y = initialPosition[1] + -1 * H * Math.sin(angle);

  car.position.x = finalPosition.x;
  car.position.y = finalPosition.y;
  car.rotation.z = angle;
});

// window.addEventListener("keyup", (key) => {
//   if (key.keyCode === 39 || key.keyCode === 37) inTurning = false;
// });
